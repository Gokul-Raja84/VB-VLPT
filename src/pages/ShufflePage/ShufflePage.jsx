...
export default function ShufflePage({ checkedInPlayers, numTeams, setNumTeams, isDark, toggleTheme }) {
  ...
  const handleShuffle = useCallback(() => {
    ...
    if (result.success) {
      ...
      const assignedIds = new Set(result.teams.flatMap(t => t.players.map(p => p.id)))
      const benchPlayers = checkedInPlayers.filter(p => !assignedIds.has(p.id))
      ...
      {teams && (
        <>
          <div ref={teamsRef} className={styles.results} key={shuffleKey}>
            {teams.map((team, i) => (
              <TeamCard key={`${shuffleKey}-${team.id}`} team={team} animationDelay={i * 80} />
            ))}
          ...
          <div className={styles.actionBar}>
            ...
            <button
              ...
              onClick={() => navigate('/court-order', { state: { teams, numTeams } })}
            >
              🏐 Court Order →
            </button>
          </div>
